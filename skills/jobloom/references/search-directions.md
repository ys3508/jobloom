...
`hard_exclusion_keywords` are hard-stop phrases, but only in the structured fields `title`, `employment_type`, `compensation_structure` and `required_certifications`. The same phrase found in `summary` or `responsibilities` routes to review instead, because prose carries negation the matcher cannot read: "we do not offer commission-only compensation" describes a salaried job. `negative_keywords` are soft demotion/review signals. `discovery_keywords` find jobs and expose skill gaps but never create CandidateFacts. `auxiliary_titles` route a job to review, with or without industry context, and never fail it.
