...
`warning_keywords` are terms the direction can work around but would rather avoid, and they are read **only from the four skill and certification lists**, never from prose. A term found in `required_skills` or `required_certifications` is one the posting itself calls mandatory: it raises `warning_term_required` and carries a ranking penalty. The same term under `preferred_skills` or `preferred_certifications` records `warning_term_preferred_only` and costs nothing — not holding a nice-to-have is not a reason to drop a posting the direction otherwise fits. Prose is excluded because "Epic required" and "familiarity with Epic is a plus" are indistinguishable to a token matcher.

A title match is an identity signal, not evidence. `route_job` compares every entry of `required_skills` against the candidate's own confirmed facts and reports `required_skill_evidence`. Routing and job evaluation use the same evidence resolver: all tokens of a requirement must resolve inside one fact whose status is explicitly `confirmed` or `locked` and whose expiry is still valid, and the result preserves that fact's evidence strength and ID. Draft, stale, conflicting, expired, malformed, or unknown-strength facts never count toward coverage. This is intentionally stricter than the former global fact-token pool. Tokens are boundary-aware, and morphology or synonyms are accepted only through a curated alias table; fuzzy matching and combining tokens across unrelated facts are forbidden. When the posting states at least three requirements and the candidate's facts cover fewer than half of them, a matching title routes to review as `title_match_without_evidence_support` instead of reaching `match` on the strength of its title alone. This stays a review, never a hard failure: coverage is measured against the fact library, so a thin library is a reason to look rather than to discard. The requirement text is returned to the caller for triage and is never written into a reason code or an event.

A target title that names its own industry ("Clinical Data Analyst") carries its context with it. A bare title that does not ("Sales Operations Analyst") must find the direction's declared `criteria.industries` elsewhere in the job — in the title, summary, responsibilities, required skills, or employer name — before it can reach `match`; otherwise it routes to review as `target_title_without_direction_context`. Only a declared industry or one of its controlled surface forms counts, never an ordinary positive keyword: "commercial analytics" and "machine learning" describe the work, not the sector, so accepting one would let a bare title match a retail or technology posting. The accepted surface forms live in `INDUSTRY_ALIASES` (for example `pharmaceuticals` also accepts `pharma` and `pharmaceutical`) and match as whole tokens, so `pharmacy` is not `pharma`. This is what keeps moving a qualifier out of a target title from silently widening the direction. A direction that declares no industries imposes no such requirement. `negative_keywords` are soft demotion/review signals. `discovery_keywords` find jobs and expose skill gaps but never create CandidateFacts. `auxiliary_titles` route a job to review, with or without industry context, and never fail it.
