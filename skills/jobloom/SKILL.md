...
- Before trusting `ranking_score`, before adding a direction keyword group, and before planning or reporting on any fill or submission work, read `references/known-liabilities.md`.
- For application questions or answer reuse, read `references/answers-and-authorization.md` and `references/schemas.md`.
- For interview stories, or for material a free-text application question might be answered from, read `references/story-bank.md`, `references/facts-and-evidence.md`, and `references/answers-and-authorization.md`. A Story is a projection of confirmed evidence, never a source of it, and it reaches an application only through `story_answers.propose`, which composes the existing field policy, stop boundary, question-form registry, and AnswerLibrary rather than deciding any of them itself.
