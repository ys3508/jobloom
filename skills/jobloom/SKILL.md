...
- For interview stories, or for material a free-text application question might be answered from, read `references/story-bank.md`, `references/facts-and-evidence.md`, and `references/answers-and-authorization.md`. A Story is a projection of confirmed evidence, never a source of it, and it reaches an application only through the AnswerLibrary — which is not built yet.
